...
class MeteoCalculator:
    ...
    def vapor_pressure_slope(
        self, airtemp: Union[float, np.ndarray]
    ) -> Union[float, np.ndarray]:
        """
    ...
    def wind_vector(
        self, speed: Union[float, np.ndarray], direction: Union[float, np.ndarray]
        ...
        vector_dir = np.where(
            vector_dir < 180,
            vector_dir + 180,
            np.where(vector_dir > 180, vector_dir - 180, vector_dir),
        )

        return magnitude, vector_dir

    def actual_vapor_pressure(
        self, airtemp: Union[float, np.ndarray], rh: Union[float, np.ndarray]
    ) -> Union[float, np.ndarray]:
        """
    ...
    def solar_parameters(
        self, doy: Union[float, np.ndarray], lat: float
    ) -> SolarResults:
        """
    ...
    def penman_monteith_reference(
        ...
        if self.config.validate_inputs:
            validate_inputs(
                airtemp=airtemp, rh=rh, airpress=airpress, rs=rs, rn=rn, g=g, u2=u2
            )
    ...
    def ra(
        self, z: float, z0: float, d: float, u: Union[float, np.ndarray]
    ) -> Union[float, np.ndarray]:
        """
    ...
    def gash79(
        self, Pg: Union[float, np.ndarray], ER: float, S: float, p: float, pt: float
    ) -> Tuple[
        Union[float, np.ndarray],
        Union[float, np.ndarray],
        Union[float, np.ndarray],
    ...
    def _validate_gash_parameters(
        self, Pg: np.ndarray, ER: float, S: float, p: float, pt: float
    ) -> None:
